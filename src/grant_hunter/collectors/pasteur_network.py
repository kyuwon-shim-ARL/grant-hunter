"""Pasteur Network SPARK / Incentive Programs collector."""
...
class PasteurNetworkCollector(BaseCollector):
    name = "pasteur_network"

    def collect(self) -> List[Grant]:
        grants: List[Grant] = []
        seen_ids: set = set()

        for url in PASTEUR_URLS:
            try:
                fetched = self._fetch_page(url, seen_ids)
                grants.extend(fetched)
                logger.info("[pasteur_network] %s -> %d grants", url, len(fetched))
                time.sleep(1)
            except Exception as exc:
                logger.error("[pasteur_network] Error fetching %s: %s", url, exc)

        logger.info("[pasteur_network] Total collected: %d grants", len(grants))
        return grants
    ...
    def _fetch_page(self, url: str, seen_ids: set) -> List[Grant]:
        ...
        # Look for program/call items
        for item in soup.find_all(["article", "div", "li"], class_=lambda c: c and any(
            kw in c.lower() for kw in ["call", "program", "fund", "grant", "spark", "incentive", "item", "post"]
        )):
    ...
    def _parse_item(self, item, base_url: str, seen_ids: set) -> Optional[Grant]:
        try:
            ...
            grant_id = f"pasteur_network::{hashlib.md5((detail_url + title).encode()).hexdigest()[:12]}"
            ...
            return Grant(
                id=grant_id,
                title=title,
                agency="Pasteur Network",
                source=self.name,
                url=detail_url,
                description=description[:2000],
                deadline=deadline,
                keywords=["Pasteur Network", "SPARK", "infectious disease", "AMR"],
                raw_data={"scraped_from": base_url},
                fetched_at=datetime.utcnow(),
            )
        except Exception as exc:
            logger.debug("[pasteur_network] parse_item error: %s", exc)
            return None
    ...
    def _parse_headings(self, soup: BeautifulSoup, base_url: str, seen_ids: set) -> List[Grant]:
        ...
        for heading in soup.find_all(["h2", "h3", "h4"]):
            ...
            if not title or len(title) < 10:
                continue
            lower = title.lower()
            if not any(kw in lower for kw in ["fund", "call", "grant", "award", "spark", "program", "incentive", "rfp"]):
                continue
